...
- 각 객체들이 직접 메시지를 주고받는 대신, 중간에 중계 역할을 수행하는 미디에이터 객체를 두고 미디에이터를 통해 각 객체들이 메시지를 주고 받을 수 있도록 해주는 디자인 패턴이다.

- 다른 협업 객체들도 모든 요청을 미디에이터에 보내고, 미디에이터는 그 요청에 맞는 객체를 실행한다.

- 아래는 비디오 플레이어에 미디에이터 패턴을 적용시킨 UML이다.

    ![mediator](https://user-images.githubusercontent.com/29175138/158027040-53a4debf-bbc5-4383-af1f-cf592db1af6b.png)

### 장점

- 개별 협업 클래스들을 수정이나 확장, 재사용하기가 쉬워진다.

### 단점

- 협업 클래스의 개수가 많아질수록 미디에이터의 코드는 복잡해져서 유지 보수하는 것이 어렵다.
